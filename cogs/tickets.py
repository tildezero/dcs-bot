...
class Tickets(commands.Cog):
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        if payload.message_id == 788271918717468673 and str(payload.emoji) == "✉️":
            ...
            await c.send(f"Hello <@{payload.member.id}>! Please answer this questions in order to recieve your carry. You may close the ticket at anytime with `d!close`")
            await c.send("Please enter the floor you need a carry for (1,2,3,4,5,6,7)! You have 60 seconds to response or else this ticket will be removed.")
            def checkfloor(m):
                return m.author == payload.member and m.channel.id == c.id and m.content in ["1","2","3","4","5","6","7"]
            try:
                msg = await self.bot.wait_for('message', check = checkfloor, timeout = 300.0)
            except asyncio.TimeoutError:
                await c.send(f"you didn't send any message in this channel for 60 seconds... closing this ticket in 5 seconds")
...
IGN - {ignmsg.content}""")
        ...
        async def on_raw_reaction_add(self, payload):
            ...
            try:
                ...
                await c.send(f"you didn't confirm this action... this ticket will not close")
